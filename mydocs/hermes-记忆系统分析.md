...
记忆系统是 Hermes Agent 的**长期记忆层**——让 Agent 在会话之间记住用户偏好、项目上下文和学到的知识，而不是每次 `/new` 之后从零开始。

### 1.2 记忆系统在 Agent 架构中的位置

记忆系统位于 AIAgent 和外部存储之间，通过 MemoryManager 统一编排：

```
AIAgent (run_agent.py)
  │
  ├── _memory_store (MemoryStore)     ← 内置：文件驱动的精选事实
  │     └── ~/.hermes/memories/{MEMORY.md, USER.md}
  │
  └── _memory_manager (MemoryManager) ← 编排器：统一所有 provider 的访问
...
| 维度 | 内置 MemoryStore | 外部 MemoryProvider（以 Honcho 为例） |
|------|-----------------|--------------------------------------|
| 存储位置 | 本地文件 `~/.hermes/memories/` | 远程 API（Honcho Cloud / 自托管） |
| 存储内容 | 精选事实条目（`§` 分隔） | 完整对话消息 + 结构化结论 + peer card |
| 谁决定存什么 | LLM 主动调用 `memory(action="add")` | 每轮自动 `sync_turn()` 同步 |
| 容量限制 | 字符硬限制（MEMORY 2200、USER 1375） | 几乎无限制（取决于后端） |
| 召回方式 | 全量注入系统提示词（冻结快照） | 语义搜索 + 推理合成 + 自动上下文注入 |
| 注入位置 | 系统提示词 `volatile` 层 | 用户消息尾部 `<memory-context>` 围栏 |
| 刷新时机 | 下个会话启动时 | 实时写入，下一轮即可召回 |
| 代码位置 | [tools/memory_tool.py](../tools/memory_tool.py) | [agent/memory_provider.py](../agent/memory_provider.py) + [agent/memory_manager.py](../agent/memory_manager.py) |
...
Hermes 将"记忆"拆分为三个递进的层次，每一层解决不同的问题：

| 层次 | 范围 | 存储机制 | 生命周期 |
|------|------|---------|---------|
| **会话内记忆** | 当前 session 的完整对话历史 | `conversation_history`（OpenAI 消息列表） | session 结束时持久化到 SQLite |
...
**本文重点**：第 2 层（会话间记忆）和第 3 层（元认知记忆），即 LLM 上下文窗口之外、需要独立持久化和检索系统的"长期记忆"。
...
Hermes 采用**窄腰 + 可插拔边缘**的架构——MemoryManager 是唯一的编排入口，内置和外部两套机制共享相同的生命周期钩子，但解决不同层面的问题：
